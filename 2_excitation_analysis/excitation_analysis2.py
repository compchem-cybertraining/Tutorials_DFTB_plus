...
###############################################################
"""
  1. Set ES software, number of states, tolerance, curr_step
"""
params = { }
...
if thermal == True:
    logfiles = glob.glob('../step2/all_logfiles/TRA*.DAT')

elif thermal == False:
    logfiles = glob.glob('../tddftb+/TRA.DAT')
...
for logfile in logfiles:
    ...
    excitation_energies, ci_basis_raw, ci_coefficients_raw_unnorm, spin_components = DFTB_methods.read_dftbplus_TRA_file( params ) 
...
for state in range(nstates):
    ...
    for sd in range( nsds ):
        ...
        for step in range( nsteps ):
            #x = len( ci_coeffs[step][state] )
            #print(F"state = {state}, sd = {sd}, step = {step}, len = {x}")
